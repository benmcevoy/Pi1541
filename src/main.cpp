...
enum EmulatingMode { IEC_COMMANDS, EMULATING_1541, EMULATING_1581 };
...
void InitialiseLCD() {
    ...
    if (i2cLcdModel) {
        int width = 128;
        int height = 64;
        if (i2cLcdModel == LCD_1306_128x32) height = 32;
        screenLCD = new ScreenLCD();
        screenLCD->Open(width, height, 1, i2cBusMaster, i2cLcdAddress,
                        i2cLcdFlip, i2cLcdModel, i2cLcdUseCBMChar);
        ...
        if ((height == 64) &&
            ...
            snprintf(tempBuffer, tempBufferSize, "Pi1541 V%d.%02d",
...
void UpdateLCD(const char* track, unsigned temperature) {
    if (screenLCD) {
        ...
        if (options.DisplayTemperature())
            snprintf(tempBuffer, tempBufferSize, "%s %02dC", track,
                     temperature);
        else
...
void UpdateScreen() {
    ...
    while (1) {
        ...
        if (value != oldMotor) {
            oldMotor = value;
            snprintf(tempBuffer, tempBufferSize, "%d", value);
            screen.PrintText(false, 12 * 8, y, tempBuffer, textColour,
                             bgColour);
            // refreshUartStatusDisplay = true;
        }
        ...
        if (value != oldATN) {
            oldATN = value;
            snprintf(tempBuffer, tempBufferSize, "%d", value);
            screen.PrintText(false, 29 * 8, y, tempBuffer, textColour,
                             bgColour);
            // refreshUartStatusDisplay = true;
        }
        ...
        if (value != oldDATA) {
            oldDATA = value;
            snprintf(tempBuffer, tempBufferSize, "%d", value);
            screen.PrintText(false, 35 * 8, y, tempBuffer, textColour,
                             bgColour);
            // refreshUartStatusDisplay = true;
        }
        ...
        if (emulating != IEC_COMMANDS) {
            // Putting the semaphore around diskCaddy.Update() keeps this core
            // awake and this breaks emulation on option B hardware. Don't know
            // why. Disabling for now.
            // #if not defined(EXPERIMENTALZERO)
            //			core0RefreshingScreen.Acquire();
            // #endif
            if (diskCaddy.Update()) caddyIndexChangedTimer = 1000;
            ...
            if (options.DisplayTemperature()) {
                if (GetTemperature(temperature)) {
                    ...
                    if (temperature != oldTemperature) {
                        ...
                        // DEBUG_LOG("%0x %d %d\r\n", temp, temp, temp / 1000);
                        snprintf(tempBuffer, tempBufferSize, "%02d",
                                 temperature);
                        screen.PrintText(false, 43 * 8, y, tempBuffer,
                                         textColour, bgColour);
                        refreshLCDStatusDisplay = true;
                    }
                }
        ...
        //	UpdateUartControls(refreshUartStatusDisplay, oldLED, oldMotor,
        //oldATN, oldDATA, oldCLOCK, oldTrack, romIndex);
...
static bool Snoop(u8 a) {
    if (a == snoopBackCommand[snoopIndex] ||
        ...
        if ((snoopIndex + 1) == sizeof(snoopBackCommand) ||
            (snoopIndex == 2 && (a == snoopBackCommand[3]))) {
...
EXIT_TYPE Emulate1541(FileBrowser* fileBrowser) {
    ...
    if (numberOfImagesMax > 10) numberOfImagesMax = 10;
    ...
    if (hash == 0x42c02586 || hash == 0x18651422 || hash == 0x2a7f4b77 ||
        hash == 0x97732c3e || hash == 0x63f809d2) {
    ...
    while (exitReason == EXIT_UNKNOWN) {
        if (refreshOutsAfterCPUStep) IEC_Bus::ReadEmulationMode1541();

        if (pi1541.m6502.SYNC())  // About to start a new instruction.
        {
            pc = pi1541.m6502.GetPC();
            // See if the emulated cpu is executing CD:_ (ie back out of
            // emulated image)
            if (snoopIndex == 0 &&
                (pc == SNOOP_CD_CBM || pc == SNOOP_CD_JIFFY_BOTH ||
        ...
        pi1541.m6502.Step();  // If the CPU reads or writes to the VIA then clk
                              // and data can change

        // To artificialy delay the outputs later into the phi2's cycle (do this
        // on future Pis that will be faster and perhaps too fast)
        ...
        // Other core will check the uart (as it is slow) (could enable uart
        // irqs - will they execute on this core?)
#if not defined(EXPERIMENTALZERO)
        ...
        if ((emulating == IEC_COMMANDS) || (resetCount > 10) || exitEmulation ||
            exitDoAutoLoad) {
            if (reset) exitReason = EXIT_RESET;
            if (exitEmulation) exitReason = EXIT_KEYBOARD;
            if (exitDoAutoLoad) exitReason = EXIT_AUTOLOAD;
        }
        ...
        do  // Sync to the 1MHz clock
        {
            ...
            if (ct > 1) {
                // If this ever occurs then we have taken too long (ie >1us) and
                // lost a cycle. Cycle accuracy is now in jeopardy. If this
                // occurs during critical communication loops then emulation can
                // fail!
                // DEBUG_LOG("!");
            }
        ...
        if (numberOfImages > 1) {
            ...
            if (nextDisk) {
                ...
                for (caddyIndex = 0; caddyIndex < numberOfImagesMax;
                     ++caddyIndex) {
                    if (inputMappings->directDiskSwapRequest &
                        (1 << caddyIndex)) {
                        DiskImage* diskImage =
                            diskCaddy.SelectImage(caddyIndex);
                        if (diskImage &&
                            diskImage != pi1541.drive.GetDiskImage()) {
                            pi1541.drive.Insert(diskImage);
                            break;
                        }
                    }
...
void emulator() {
    ...
    m_IEC_Commands.SetLowercaseBrowseModeFilenames(
            ...
            if (!options.GetDisableSD2IECCommands()) {
                ...
                while (emulating == IEC_COMMANDS) {
                    IEC_Commands::UpdateAction updateAction =
                        m_IEC_Commands.SimulateIECUpdate();
                    ...
                    switch (updateAction) {
                        ...
                        case IEC_Commands::NONE:
                            ...
                            if (fileBrowser->SelectionsMade())
                                emulating = BeginEmulating(
                                    fileBrowser,
                                    fileBrowser->LastSelectionName());
                            break;
                        case IEC_Commands::IMAGE_SELECTED:
                            // Check selections made via IEC commands (like
                            // fb64)

                            fileBrowserSelectedName =
                                m_IEC_Commands.GetNameOfImageSelected();

                            if (DiskImage::IsLSTExtention(
                                    fileBrowserSelectedName)) {
                                if (fileBrowser->SelectLST(
                                        fileBrowserSelectedName)) {
                                    emulating = BeginEmulating(
                                        fileBrowser, fileBrowserSelectedName);
                                } else {
                                ...
                                const FILINFO* filInfoSelected =
                                    m_IEC_Commands.GetImageSelected();
                                DEBUG_LOG("IEC mounting %s\r\n",
                                          filInfoSelected->fname);
                                bool readOnly =
                                    (filInfoSelected->fattrib & AM_RDO) != 0;

                                if (diskCaddy.Insert(filInfoSelected, readOnly))
                                    emulating = BeginEmulating(
                                        fileBrowser, filInfoSelected->fname);
                                else
                                    fileBrowserSelectedName = 0;
                            } else {
                                fileBrowserSelectedName = 0;
                ...
                while (emulating == IEC_COMMANDS) {
                    ...
                    if (fileBrowser->SelectionsMade())
                        emulating = BeginEmulating(
                            fileBrowser, fileBrowser->LastSelectionName());
                    usDelay(1);
                }
            }
        } else {
...
static bool AttemptToLoadROM(char* ROMName) {
    ...
    if ((FR_OK == f_open(&fp, ROMName, FA_READ)) ||
        ...
        DEBUG_LOG("Opened ROM %s %d %d %d\r\n", ROMName, ROMs::ROM_SIZE,
                  bytesRead, bytesRead == ROMs::ROM_SIZE);
        return true;
    } else {
...
static void DisplayLogo() {
    ...
    snprintf(tempBuffer, tempBufferSize, "V%d.%02d", versionMajor,
             versionMinor);
...
void DisplayOptions(int y_pos) {
    ...
    screen.PrintText(false, 0, y_pos += 16, tempBuffer, COLOUR_WHITE,
                     COLOUR_BLACK);
    snprintf(tempBuACTLED_ACTIVE_LOWns.GetAutoBaseName());
    screen.PrintText(false, 0, y_pos += 16, tempBuffer, COLOUR_WHITE,
                     COLOUR_BLACK);
#endif
}
...
void DisplayI2CScan(int y_pos) {
    ...
    for (int address = 0; address < 128; address++) {
        if (RPI_I2CScan(BSCMaster, address)) {
            ptr += snprintf(tempBuffer + ptr, tempBufferSize - ptr, "%3d ",
                            address);
            count++;
        }
    }
...
static void CheckOptions() {
    ...
    if (ROMName1581) {
        ...
        if ((FR_OK == f_open(&fp, ROMName1581, FA_READ))) {
            ...
            SetACTLed(true);
            res =
                f_read(&fp, roms.ROMImage1581, ROMs::ROM1581_SIZE, &bytesRead);
            SetACTLed(false);
    ...
    for (ROMIndex = ROMs::MAX_ROMS - 1; ROMIndex >= 0; --ROMIndex) {
        ...
        if ((FR_OK == f_open(&fp, ROMName, FA_READ)) ||
            ...
            SetACTLed(true);
            res = f_read(&fp, roms.ROMImages[ROMIndex], ROMs::ROM_SIZE,
                         &bytesRead);
            SetACTLed(false);
    ...
    if (roms.ROMValid[0] == false &&
        !(AttemptToLoadROM("d1541.rom") || AttemptToLoadROM("dos1541") ||
          AttemptToLoadROM("d1541II") || AttemptToLoadROM("Jiffy.bin"))) {
        snprintf(tempBuffer, tempBufferSize,
                 "No ROM file found!\r\nPlease copy a valid 1541 ROM file in "
...
void Reboot_Pi() {
    if (screenLCD) screenLCD->ClearInit(0);
    reboot_now();
}
...
void UpdateFirmwareToSD() {
    ...
    if (SwitchDrive("USB01:")) {
        ...
        if (f_getcwd(cwd, 1024) == FR_OK) {
            ...
            bool found =
                f_findfirst(&dir, &filInfo, ".", firmwareName) == FR_OK;
                        ...
                        if ((res == FR_OK) && ((u32)filInfo.fsize == bytes)) {
                            if (SwitchDrive("SD:")) {
                                if (f_chdir("\\") == FR_OK) {
                                    bool same = true;
                                    if (FR_OK ==
                                        f_open(&fp, firmwareName, FA_READ)) {
                                        char* ptr = mem;
                                        char buffer[256];
                                        unsigned bufferIndex = 0;
                                        unsigned bytesRead;
                                        do {
                                            f_read(&fp, buffer, 256,
                                                   &bytesRead);

                                            for (unsigned index = 0;
                                                 index < bytesRead; ++index) {
                                                if (buffer[index] !=
                                                    mem[bufferIndex + index]) {
                                                    same = false;
                                                    break;
                                                }
                                            }
                                            bufferIndex += bytesRead;
                                        } while (same && (bufferIndex <
                                                          (u32)filInfo.fsize));
                                        f_close(&fp);
                                    }
                                    ...
                                    if (!same &&
                                        (FR_OK ==
                                         f_open(&fp, firmwareName,
                                                FA_CREATE_ALWAYS | FA_WRITE))) {
                                        snprintf(tempBuffer, tempBufferSize,
                                                 "Updating firmware.\r\n");
                                        screen.MeasureText(false, tempBuffer,
                                                           &widthText,
                                                           &heightText);
                                        xpos = (widthScreen - widthText) >> 1;
                                        ypos = (heightScreen - heightText) >> 1;
                                        screen.PrintText(
                                            false, xpos, ypos, tempBuffer,
                                            COLOUR_WHITE, COLOUR_RED);

                                        res =
                                            f_write(&fp, mem,
                                                    (u32)filInfo.fsize, &bytes);
                                        f_close(&fp);
                                    }
                                }
                            }
...
void kernel_main(unsigned int r0, unsigned int r1, unsigned int atags) {
    ...
    if (options.I2CScan()) DisplayI2CScan(y_pos += 32);

    if (options.ShowOptions()) DisplayOptions(y_pos += 32);
    ...
    if (numberOfUSBMassStorageDevices > 0) {
        if (SwitchDrive("USB01:")) UpdateFirmwareToSD();
    }
#endif
    f_chdir("/1541");
    ...
    // TODO: BM trying to debug, does this even run?
    // lets blink the led instead
    while (1) {
        Set_ACT_LED(true);
        MsDelay(500);
        Set_ACT_LED(false);
